...
class Jsdoc extends BaseTask {
  executor (resolve, reject) {
    ...
    jsdoc
      .explain(options)
      .then(explainOutput => fs.writeFile(path.resolve(folder, '1-jsdoc.json'), JSON.stringify(explainOutput, null, '  ')))
      .then(resolve)
      .catch(reject)
  }
}
...
class Dmd2 extends BaseTask {
  executor (resolve, reject) {
    ...
    const inputFile = path.resolve(folder, '5-jsdoc-parse2.json')
