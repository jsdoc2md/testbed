...
class JsdocParse2 extends BaseTask {
  executor (resolve, reject) {
    ...
    fs.readFile(path.resolve(folder, '1-jsdoc.json'), 'utf8')
      ...
      .then(data => fs.writeFile(path.resolve(folder, '4-jsdoc-parse2.json'), JSON.stringify(data, null, '  ')))
      .then(resolve)
      .catch(reject)
  }
}
...
class Dmd extends BaseTask {
  executor (resolve, reject) {
    ...
    const stream = inputStream
      ...
      .on('error', reject)
      .on('readable', () => {
        stream.pipe(this.getOutputStream('3-dmd.md'))
          .on('close', resolve)
      })
  }
}
...
class Dmd2 extends BaseTask {
  executor (resolve, reject) {
    ...
    fs.readFile(inputFile, 'utf8')
      ...
      .then(output => fs.writeFile(path.resolve(this.context.folder, '5-dmd2.md'), output))
      .catch(reject)
      .then(resolve)
  }
}
